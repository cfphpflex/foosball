<?php
 ...
 class getplayerdata extends CI_Model {
  ...
  //dbmodel
   public function getplayerdatabyplayer($player){
        try{   
			 ...
			 try{
		        ...
		        // sort by
				 ...
				 $sql = $this->_db->prepare("  
									 select player1,player1score, player2, player2score  
									   from import 	
									   
									   where player1 in ( select player from ranking where 
									     ranking_id =  {$player}  )
									   or player2 in ( select player from ranking where 
									     ranking_id =  {$player}  )
