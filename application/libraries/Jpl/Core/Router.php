...
class Router
{
    ...
    {
        $route = (isset($_SERVER['REQUEST_URI']) && $_SERVER['REQUEST_URI'] != "/") ? trim($_SERVER['REQUEST_URI']) : '/index/index';
        /**
         * Try to match a defined route, if unable, try and match an organic
         * route
         */
        $matchedRoute = self::_getMatchingRoute($route);
        $routeArray = array_slice(explode('/', $route),1);
        ...
        if ($matchedRoute != false) {
            ...
            $actionPart = ($routeArray[1] == "" || !isset($routeArray[1])) ? 'index' : $routeArray[1];
        }
